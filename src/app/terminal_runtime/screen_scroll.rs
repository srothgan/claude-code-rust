    ...
    );
    {
        ...
        match request.direction {
            ...
            ScreenScrollDirection::Down => {
                queue!(writer, ScrollDown(applied_rows))?;
            }
        }
...
fn scroll_adjusted_cursor_position(
    ...
    let y = match direction {
        ...
        ScreenScrollDirection::Down => cursor_pos.y.saturating_add(rows),
    };
...
mod tests {
    ...
    fn adjusted_cursor_tracks_screen_content() {
        ...
        assert_eq!(
            scroll_adjusted_cursor_position(Position::new(4, 10), ScreenScrollDirection::Down, 3),
            Position::new(4, 13)
        );
    }
    ...
    fn scroll_down_writes_command_and_updates_bookkeeping() {
        ...
        let outcome = scroll_screen(&mut terminal, ScreenScrollRequest::down(3)).unwrap();

        assert_eq!(outcome.applied_rows, 3);
        assert_eq!(terminal.last_known_cursor_pos, Position::new(5, 12));
        assert_eq!(terminal.history_bounds(), Some((7, 13)));
        assert!(!terminal.backend_mut().written.is_empty());
    }
